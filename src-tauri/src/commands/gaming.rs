...
/// Đọc giá trị registry dạng string.
pub(crate) fn reg_val(hive: &str, path: &str, name: &str) -> String {
    let ps = format!(
        r#"
$v = Get-ItemProperty -Path '{0}\{1}' -Name '{2}' -ErrorAction SilentlyContinue
if ($v) {{ [string]$v.'{2}' }} else {{ '' }}
"#,
        hive, path, name
    );
    ps_quiet(&ps, 10).trim().to_string()
}

const HKLM_PERF: &str = "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile";
const HKLM_GFX: &str = "HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers";
const HKLM_DWM: &str = "HKLM:\\SOFTWARE\\Microsoft\\Windows\\Dwm";
const HKLM_PRIO: &str = "HKLM:\\SYSTEM\\CurrentControlSet\\Control\\PriorityControl";
const HKLM_MM_GAMES: &str = "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games";
...
/// Trả về danh sách tweak + trạng thái hiện tại.
#[tauri::command]
pub fn get_gaming_tweaks() -> Vec<GamingTweak> {
    let hags_on = reg_val(HKLM_GFX, "", "HwSchMode") == "2";
    let mpo_off = reg_val(HKLM_DWM, "", "OverlayTestMode") == "5";
    let game_mode = reg_val("HKCU:\\Software\\Microsoft\\GameBar", "", "AutoGameModeEnabled") == "1";
    let gamedvr = reg_val("HKCU:\\System\\GameConfigStore", "", "GameDVR_FSEBehaviorMode") == "2";
    let nagle = {
        let out = ps_quiet(
    ...
    let net_throttle = reg_val(HKLM_PERF, "", "NetworkThrottlingIndex") == "4294967295";
    let sys_resp = reg_val(HKLM_PERF, "", "SystemResponsiveness") == "0";
    let ult_power = power_plan_active().to_lowercase().contains("e9a42b02");
    let vbs_off = {
        let out = ps_quiet("bcdedit /enum '{current}'", 10).to_lowercase();
    ...
    // Win32PrioritySeparation = 38 (hex 0x26, foreground boost + short quantum)
    let win32prio = reg_val(HKLM_PRIO, "", "Win32PrioritySeparation") == "38";
    // MMCSS Games: Priority = 6 (68 decimal? PS trả số) — kiểm tra priority >= 6
    let mmcss = {
        let v = reg_val(HKLM_MM_GAMES, "", "Priority");
        v.parse::<u32>().map(|x| x >= 6).unwrap_or(false)
    };
    // TDR delay = 0 (disable timeout)
    let tdr = reg_val(HKLM_GFX, "", "TdrDelay") == "0";
    // Mouse accel off: MouseSpeed = 0
    let mouse = reg_val("HKCU:\\Control Panel\\Mouse", "", "MouseSpeed") == "0";
...
#[tauri::command]
pub fn apply_gaming_tweak(id: String, action: String) -> Result<(bool, String), String> {
    ...
    let _out = ps_quiet(script, 20);
    Ok((true, format!("{} '{}' xong.", if action == "apply" { "Bật" } else { "Tắt" }, {
        get_gaming_tweaks().into_iter().find(|t| t.id == id).map(|t| t.label).unwrap_or(id)
    })))
}
